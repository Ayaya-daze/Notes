...
$$
P(d_{i}=0\mid I)=P(d_{i}=1\mid I)=\frac{1}{2}\quad P(\mathbf{d}^{(n)}\mid I)=2^{-n}
$$

这样给定前 $n-1$ 次结果 $\mathbf{d}^{(n-1)}$

$$
P(\mathbf{d}_{(n)}\mid \mathbf{d}^{(n-1)},I)=
$$

即

$$
P(\mathbf{d}_{(n)}\mid \mathbf{d}^{(n-1)},I) = P(d_{n}\mid I)=\frac{1}{2}
$$
...
$$
P(n\mid N,f,I)= P(\sum_{i=1}^Nd_{i} = n \mid N ,f ,I) = \frac{N!}{n!(N-n)!}f^n(1-f)^{N-n}
$$
...
现在我们知道通量 $\phi$ , 探测器的效率 $\eta$ , 则探测器计数 $c$ 的概率为

$$
p(c\mid \eta ,\phi)=\sum_{n=0}^\infty p(c\mid n, \eta,\phi)p(n\mid \eta,\phi)=\sum p(c\mid n,\eta)p(n\mid \phi)=\frac{(\eta \phi)^c}{c!}e^{-\eta\phi}
$$

这里是考虑了因果关系 $\phi\to n\overset{\eta}{\to} c$
即一个新的 poisson 分布
...
$$
S/N=\frac{\phi}{\sqrt{\phi_{sky}}}\sqrt{t}
$$

此时更加困难, 由于 $\phi_{sky}>\phi$ , 所以应该在更暗的时候观测

Gaussian 分布

$$
p(x\mid\mu,\sigma)=\frac{1}{\sqrt{2\pi}\sigma}\exp(-\frac{(x-\mu)^2}{2\sigma^2})=\mathcal{N}(\mu,\sigma)
$$
...
$$
\rho(x,y)\mathrm{d}x\mathrm{d}y=f(x)\mathrm{d}xf(y)\mathrm{d}y
$$

* isotropy : 概率分布旋转不变 
	$$\rho(x,y)\mathrm{d}x\mathrm{d}y=g(r,\theta)r\mathrm{d}r\mathrm{d}\theta$$
	
	则$g(r,\theta)=g(\sqrt{x^2+y^2})$
...
均匀采样纵坐标 $u\in[0,1)$ , 计算反函数 $x=F^{-1}(u)$
...
$$
G(y)=F[r^{-1}(y)]\quad g(y)=p(r^{-1}(y))\frac{\mathrm{d}r^{-1}(y)}{\mathrm{d}y}
$$

*  若 $r(x)$ 是单调减的

$$
G(y)=1-F[r^{-1}(y)]\quad g(y)=-p(r^{-1}(y))\frac{\mathrm{d}r^{-1}(y)}{\mathrm{d}y}
$$

* 统一地

$$
g(y)=p(r^{-1}(y))|\frac{\mathrm{d}r^{-1}(y)}{\mathrm{d}y}|
$$
...
$$
r\exp(-\frac{r^2}{2})\mathrm{d}r\mathrm{d}\theta
$$

这样就有可积的 cdf 了

$$
u\sim \exp(-\frac{r^2}{2})\quad v \sim\frac{\theta}{2\pi}
$$
