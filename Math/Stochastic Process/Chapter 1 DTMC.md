...
$$
i=x_0\to x_1\to \cdots \to x_k=j
$$

于是中间不会再次经过 $i$ , 并且

$$
p(x_0,x_1)\cdots p(x_{k-1},x_k)>0
$$

$$
P_i(T_i=\infty)\geq p(x_0,x_1)\cdots p(x_{k-1},x_k)\,P_j(T_i=\infty)>0
$$

> 直觉是这个态应该会从 $j$ 处泄漏出去, 所以应该先刻画到 $j$ 态的情况, 给出下界

那么有 $\rho_{ii}<1$ 即为 transient state (因为有非 0 的无穷长时间的概率)
...
$$
\sum_{n=1}^\infty p^n(j,j)\geq \sum_{n=1}^\infty p^{m}(j,i)p^n(i,i)p^k(i,j)=\infty
$$
...
这把问题放回到了二元版本: 只要 $\gcd(\phi(u),A)=1$ , 就会存在充分大的正数都能写成这样的线性组合
...
$$
\text{若链不可约且 } i \text{ 是 positive recurrent, 则 }\frac{\mu_i(j)}{\mathbb{E}_i T_i}\text{ is stationary},\quad 0<\mu_{i}(j)<+\infty
$$

当然这里要求 $\mathbb{E}_i T_i <\infty$ , 即 $i$ 是正常返态
即 $\pi$ 存在
...
#### Theorem
若 $P_c(V_{A\cup B}<\infty)=1$ 则 $h(c)=P_c(V_A<V_B)$
...
#### Theorem
若 $(I)$ 则 all states are positive recurrent / null recurrent / transient
...
![[attachments/tikz/random-process-ch1-branching-tree.png]]
...
>注意这里每个 $Z_{n,i}$ 都是独立同分布的, 并且 $n$ 只是一个 label
...
首先是状态分类: $0$ 是唯一的吸收态; 在非退化的 offspring law 下, 正整数态都是 transient state

记 $\mu =\mathbb{E}Z_{n,1}$ 那么

$$
\mathbb{E}(X_1)=\sum_{j}\mathbb{E}(X_1|X_0=j)P(X_0=j)=\mu \mathbb{E}X_0
$$
...
首先如果 $\mu<1$ , 这表明期望随指数衰减到 0 , 再用 Markov 不等式

$$
P(X_n>0)\le \mathbb{E}[X_n]\to 0
$$
...
$$
\rho=\sum_{k}p_k \rho^k=\phi(\rho)
$$

这里

$$
\phi(s)=\mathbb{E}[s^Z]=\sum_k p_k s^k
$$

这是 offspring distribution 的 probability generating function

其中 $\phi(1)=1$ , 现在分析 $\rho$ 是方程的哪一支解

先看 $X_n$ 的 generating function

$$
\Pi_n(s)=\mathbb{E}[s^{X_n}]=\sum_j \mathbb{E}(s^{X_n}\mid X_{n-1}=j)P(X_{n-1}=j)
$$

而 

$$
X_{n}\mid X_{n-1}=j\sim Z_{n,1}+\cdots Z_{n,j}
$$
...
$$
\rho_n=\sum_k P_1(X_1=k)\rho_{n-1}^k=\sum_k p_k \rho_{n-1}^k=\phi(\rho_{n-1})
$$

这是一个单增, 且有界的序列, 极限即为 $\rho$ , 即

$$
\rho=P_1\left(\bigcup_n\{X_n=0\}\right)
$$

实际上在找 $\phi(\rho)$ 的 fix point , 按照走法, 它应该走到最小的解就不动了

#### Lemma

$$
\rho=\min\{s\geq 0:\phi(s)=s\}
$$

只需要证明所有不动点都比 $\rho$ 大即可, 对任意 $s\geq 0$

首先 $\rho_0=0\leq s$ , 从而 $\rho_1=\phi(\rho_0)\leq \phi(s)=s$ , 进一步归纳就得到

$$
\rho_n\leq s\quad \forall n
$$

而 $\phi'(1)=\mu$ , 因为

$$
\phi'(s)=\sum_k k p_k s^{k-1}
$$

在非退化情形 $p_1\neq 1$ 下, 由 $\phi$ 的凸性可以读出

$$
\mu\le 1 \implies \rho=1,
\qquad
\mu>1 \implies \rho<1
$$

Codex 补充：下面这张图画的是 $\mu>1$ 的典型情形. 此时 $y=\phi(s)$ 与 $y=s$ 在 $[0,1]$ 上有两个交点, 较小的那个交点就是 extinction probability $\rho$ .

![[attachments/tikz/random-process-ch1-fixed-point.png]]
